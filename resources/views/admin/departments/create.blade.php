...
<div class="col-12 grid-margin">
    <div class="card">
        <div class="card-body">
            <h4 class="card-title">{{ __('dashboard.Create_Department') }}</h4>
            <form method="POST" action="{{ route('departments.store') }}" class="form-sample">
                @csrf
                        ...
                        <div class="form-group row">
                            ...
                            <select name="parent_id" value="{{ old('parent_id') }}" class="js-example-basic-single col-sm-9" style="padding:auto !important;">
                                ...
                                @forelse($departments as $department)
                                    <option value="{{ $department->id }}" {{ isset($parent_id) && ( $parent_id == $department->id ) ? 'selected':null }}>{{ $department->name }}</option>
                                @empty
                                @endforelse
                            </select>
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group row">
                            ...
                            <select name="have_child_departments" class="js-example-basic-single col-sm-9" style="padding:auto !important;" required>
                                <option value="1" > {{__('dashboard.other_departments')}}</option>
                                <option value="0" > {{ __('dashboard.Certificates') }}</option>
                            </select>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <button type="submit" class="btn btn-danger btn-icon-text">
                            ...
                            {{ __('dashboard.Submit_department') }}
                        </button>
                    </div>
                </div>
            </form>
