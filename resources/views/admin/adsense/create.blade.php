...
<div class="col-12 grid-margin" id="app">
    <div class="card">
        <div class="card-body">
            <h4 class="card-title">{{__('dashboard.Create_New_Adsense')}}</h4>
            <form method="POST" action="{{ route('adsenses.store') }}" class="form-sample" enctype="multipart/form-data">
                @csrf
                <!-- show errors  -->
                <div class="row">
                    @if($errors->any())
                            ...
                            <div class="col-sm-9">
                                <select name="page_name" class="form-control" required>
                                    @forelse(Config('settings.pages') as $page)
                                        <option value="{{ $page ?? null }}">{{ $page ?? null }}</option>
                                    @empty
                                    @endforelse
                                </select>
                            </div>
                            ...
                            <div class="col-sm-9">
                                <select name="position" class="form-control" required>
                                    @forelse(Config('settings.positions') as $key => $page)
                                        <option value="{{ $key ?? null }}">{{ __('dashboard.'.$page) ?? null }}</option>
                                    @empty
                                    @endforelse
                                </select>
                            </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <textarea rows="5" cols="45" type="text" name="code_adsense" class="form-control"  placeholder="put code here"></textarea>
                            </div>
                        </div>
                    </div>
