...
<div class="col-12 grid-margin" id="app">
    <div class="card">
        <div class="card-body">
            <h4 class="card-title">{{__('dashboard.Edit_Font')}}</h4>
            <form method="POST" action="{{ route('fonts.update',$font->id) }}" class="form-sample" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input type="text" name="name" value="{{ $font->name ?? null }}" class="form-control" required>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <textarea name="font_family" placeholder="'Cairo', sans-serif;" class="form-control" required>{{ $font->font_family }}</textarea>
                            </div>
                        </div>    
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input type="text" name="cdn_font_url" placeholder="https://fonts.googleapis.com/css2?family=Cairo:wght@200&display=swap" value="{{ $font->cdn_font_url ?? null }}" class="form-control">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <image-upload saved-image="{{ $font->image ? $font->image : 'null' }}" :file-image="true" title="{{__('dashboard.Upload_Font_File')}}"></image-upload>
                </div>
                <div class="row">
                    <div class="col-md-6">
                        <button type="submit" class="btn btn-danger btn-icon-text">
                            <!-- <i class="ti-file btn-icon-prepend"></i> -->
                            {{__('dashboard.edit_Submit_font')}}
                        </button>
                    </div>
                </div>
            </form>
